<script lang="ts">
	import { cn } from '$lib/components/animation/utils';
	...
	// Removed text prop as we're using slots instead
	export let colors = {
		first: '#9E7AFF',
		second: '#FE8BBB'
	};
	...
	onMount(async () => {
		// Only import and use svelte-motion on the client side
		if (browser) {
			try {
				// Dynamic import of Motion component
				const sveltemotion = await import('svelte-motion');
				Motion = sveltemotion.Motion;
				motionLoaded = true;
			} catch (error) {
				console.error("Failed to load Motion from svelte-motion:", error);
			}
		}
...
>
	<span class="relative inline-block">
		<!-- Sparkles layer (positioned absolutely, with pointer-events-none to allow clicking through) -->
		{#if browser && motionLoaded}
			{#each sparkles as item}
				...
				>
					...
					>
						<svg id={item.id} width="21" height="21" viewBox="0 0 21 21">
							<path
